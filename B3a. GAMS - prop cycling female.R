...
# or in viz, integration higher as time estrlen
...
a <- f_mixed_dir_sna_w %>%
  ...
  labs(x = "Age", title = "Time estrlen by age", subtitle = "size of dot = time receiving grooming", y = "Annual time estrlen") +
...
# special kind of interaction, see https://www.youtube.com/watch?v=sgw4cu8hrZM at 1:19g
...
# prop cyc as main effect and independent interaction w age ------
##  Grooming -----
# -- in out deg strength ----
...
# -- social role ------
ii_trg_f_estr <- gam(bt ~ s(age_mid_year, k =5) + s(prop_cyc, k = 5) + ti(age_mid_year, prop_cyc, k = 5) + s(chimp_id, bs = "re"), data = f_mixed_sna_w  %>% filter(behavior == "total_grooming"), family = gaussian(link = "log"), method = "REML")
...
# C. tensor smooths: time estrlen + age (grooming) ------- 
# -- in out deg strength ----
