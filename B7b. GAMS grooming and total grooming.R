...
dig_f_cyc <- gam(deg_in ~ s(age_mid_year, prop_cyc, k = 5) + s(avg_rank, k = 5)  + s(chimp_id, bs = "re"), data = f_mixed_dir_sna_uw, family = gaussian(link = "log"), method = "REML")
...
sig_f_cyc <- gam(deg_in ~ s(age_mid_year, prop_cyc, k = 5) + s(avg_rank, k = 5)  + s(chimp_id, bs = "re"), data = f_mixed_dir_sna_w, family = gaussian(link = "log"), method = "REML")
...
f_sog <- gam(deg_out ~ s(age_mid_year, k = 5) + s(chimp_id, bs = "re"), data = f_same_dir_sna_w, family = Gamma(link = "log"), method = "REML")
...
f_trg <- gam(trans ~ s(age_mid_year, k = 5)  + s(chimp_id, bs = "re"), data = f_same_sna_w %>% filter(behavior == "total_grooming"), family = gaussian(link = "log"), method = "REML")
...
ecgr <- gam(ec ~ s(age_mid_year, by = sex, k = 5) + s(avg_rank, by = sex, k = 5)  + s(chimp_id, bs = "re"), data = mixed_sna_w %>% filter(behavior == "total_grooming"), family = gaussian(link = "log"), method = "REML")
