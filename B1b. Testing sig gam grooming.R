...
e <- test_sig_gamm_mixed(response = "Betweenness", behavior = "Grooming", 
                         mod = btg, modr = btgr, F_ran = F_ran_btg, F_ran_r = F_ran_btgr,
                         beta_ran = betaR_ran_btg, beta_ran_r = betaR_ran_btgr)

f <- test_sig_gamm_mixed(response = "Local Transitivty", behavior = "Grooming",
...
for(i in 1:1000){
  ...
  ran_msdig_list[[i]] <- gam(deg_in ~ s(age_mid_year, k = 5) + s(chimp_id, bs = "re"),  data = data_uw %>% filter(sex == "M"), family = gaussian(link = "log"), method = "REML")
  ran_msdigr_list[[i]] <- gam(deg_in ~ s(age_mid_year, k = 5) + s(avg_rank, k = 5) + s(chimp_id, bs = "re"),  data = data_uw %>% filter(sex == "M"), family = gaussian(link = "log"), method = "REML")
...
#      file = "data/models gam - ran - H1 same sex grooming random deg and In-Strength.Rdata")

#       - extract Fs and R -------
load("data/models gam - ran - H1 same sex grooming random deg and In-Strength.Rdata", verbose = T)
...
for(i in 1:1000){
  ...
  ran_msdogr_list[[i]] <- gam(deg_out ~ s(age_mid_year, k = 5) + s(avg_rank, k = 5) + s(chimp_id, bs = "re"),  data = data_uw %>% filter(sex == "M"), family = gaussian(link = "log"), method = "REML")
...
save(ran_fsdog_list, ran_fsdogr_list, ran_fssog_list, ran_fssogr_list,
     ...
     file = "data/models gam - ran - H1 same sex grooming random deg and Out-Strength.Rdata")
...
#   4. Evaluating sig grooming mixed - comparing observed to random --------
