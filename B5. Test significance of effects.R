...
#save(sig_tests, same_sex_results, mixed_sex_results, file = "data/sig tests - all linear models.Rdata")

# 1a. Calc sig for non-linear models ------
load("data/random coefs - non linear models.Rdata", verbose = T)
load("data/models - non linear age relationships.Rdata", verbose = T)

# function make tables of non linear sig
non_linear_sig <- function(mod, response = c("bt","ec","deg", "deg_in", "deg_out", "trans")){

#mod = f_gm_mixed_deg_in_uw

#create randomized coef df name
mod_name = deparse(substitute(mod))
ran = get(paste0(mod_name, "_ran_sig"))
f_pred_list = c("age", "age_squared", "rank", "prop_cyc")
m_pred_list = c("age", "age_squared", "rank")

#pred_list
if(grepl("^f", mod_name)){ pred_list = f_pred_list}
if(grepl("^m", mod_name)){ pred_list = m_pred_list}
if(grepl("^both", mod_name)){ pred_list = m_pred_list}
...
#storage for sig of predictors
pred_list_sig <- vector("list", length = length(pred_list))
names(pred_list_sig) <- pred_list

for(i in seq(pred_list)){
  pred <- pred_list[[i]]
  beta <- ex_coef_single(mod, pred)
names(beta) <- NULL
  
  if(!is.na(beta)){
    prop_greater <- sum(ex_coef_single(mod, pred) > ran[pred]) / sum(!is.na(ran[pred])) 
    if(all(prop_greater >= 0.95 | prop_greater <= 0.05)){sig = "*"} else {sig = ""}  
  }  else { beta = NA ; prop_greater = NA ; sig = NA }
  
  pred_list_sig[[i]] <- data.frame(mod_name = mod_name, response = response, pred = pred, sex = sex, beh = beh, net_sex = net_sex, weighted = weighted, beta = beta, prop_greater = prop_greater, sig = sig)
  
  }

non_linear_sig <- do.call("rbind", pred_list_sig)

#return(mod_name)
return(non_linear_sig)

}

a <- non_linear_sig(f_gm_mixed_deg_in_uw, response = "deg_in")
b<- non_linear_sig(f_gmgmd_mixed_trans_w, response = "trans")
c <- non_linear_sig(f_prox_mixed_ec_w, response = "ec")
d <- non_linear_sig(m_gmgmd_mixed_ec_w, response = "ec")

non_linear_raw <- do.call("rbind", list(a,b,c,d))

#save(non_linear_raw, file = "data/sig tests - non linear models.Rdata")

# 2. Explore sig ------
load("data/sig tests - all linear models.Rdata", verbose = T)
...
# NON LINEAR - various to include in tables of all 3 hypos.
non_lin_tab <- table_results(non_linear_raw, table = "full")
non_lin_tab
...
# write.table(non_lin_tab, file = "results/tables/ non linear age sex effects table.txt", row.names = F, quote = F, sep = "/")

#H1
ss_undir_deg_tab <- table_results(same_sex_results, "DEG")
#Hone degree table... Only use directed? refer to hypos
ss_undir_deg_tab

ss_dir_deg_tab <- table_results(same_sex_results, "DIR DEG")
# could potentially only use undir degree for prox...
ss_dir_deg_tab

#H2
ss_bt_trans_tab <- table_results(same_sex_results, "BT")
ss_bt_trans_tab

#H3
ss_ec_tab <- table_results(same_sex_results, "EC")
ss_ec_tab

#write.table(ss_undir_deg_tab, file = "results/tables/same sex/H1a. undir deg - age sex effects table.txt", row.names = F, quote = F, sep = "/")
#write.table(ss_dir_deg_tab, file = "results/tables/same sex/H1b. dir deg - age sex effects table.txt", row.names = F, quote = F, sep = "/")
#write.table(ss_bt_trans_tab, file = "results/tables/same sex/H2. bt trans - age sex effects table.txt", row.names = F, quote = F, sep = "/")
#write.table(ss_ec_tab, file = "results/tables/same sex/H3. ec - age sex effects table.txt", row.names = F, quote = F, sep = "/")
